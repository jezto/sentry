...
Kindly follow the server rules and be respectful to others!`,
  ...
  let result = await lib.discord.guilds.members.destroy({
    user_id: `${userId}`,
  ...
  await lib.discord.channels.messages.create({
    ...
    embed: {
      type: 'rich',
      title: 'User Kicked!',
      description: `<@${userId}> has been kicked for the reason: **${reason}** by <@${context.params.event.member.user.id}>
  ...
  await lib.discord.channels.messages.create({
    channel_id: `${context.params.event.channel_id}`,
    content: `<@${context.params.event.member.user.id}> - You don't hold the required permission to kick members!`,
  });
}
